// t33: SAS similarity – equal included angle + proportional adjacent sides
const DiagramTriangleSimilaritySas = (() => {
  ...
  function render(cfg={}){
    ...
    function sideLabel(p1,p2,text,gcx,gcy,offset=22){
      const mx=(p1.x+p2.x)/2, my=(p1.y+p2.y)/2;
      const dx=mx-gcx, dy=my-gcy, len=Math.hypot(dx,dy);
      return GeoRenderer.label(Geo.point(mx+(dx/len)*offset, my+(dy/len)*offset),text,{color:blue,size:22,bold:true});
    }
    ...
    // equal included angle at A (between AB and AC)
    parts.push(GeoRenderer.angleArc(va(A1,B1,C1,32),{color:red,strokeOnly:true,strokeWidth:2}));
    parts.push(GeoRenderer.angleArc(va(A2,B2,C2,24),{color:red,strokeOnly:true,strokeWidth:2}));
    // letter labels: lowercase for large triangle, uppercase for small triangle
    parts.push(sideLabel(A1,B1,'a',cx1,cy1));
    parts.push(sideLabel(A1,C1,'b',cx1,cy1));
    parts.push(sideLabel(A2,B2,'A',cx2,cy2));
    parts.push(sideLabel(A2,C2,'B',cx2,cy2));
    // similarity symbol
