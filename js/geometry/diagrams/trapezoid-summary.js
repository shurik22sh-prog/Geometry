const DiagramTrapezoidSummary = (() => {
  function render(cfg={}) {
    ...
    function inwardLabel(cx, cy, V, text, offset, size, nudgeY=0, nudgeX=0) {
      const dx=cx-V.x, dy=cy-V.y, len=Math.hypot(dx,dy);
      const x=(V.x+(dx/len)*offset+nudgeX).toFixed(1), y=(V.y+(dy/len)*offset+nudgeY).toFixed(1);
    ...
    // Angle labels on main trapezoid
    const mcx=(TL.x+TR.x+BR.x+BL.x)/4, mcy=(TL.y+TR.y+BR.y+BL.y)/4;
    parts.push(inwardLabel(mcx,mcy, BL, 'α',      30, 22));
    parts.push(inwardLabel(mcx,mcy, BR, 'β',      42, 22));
    parts.push(inwardLabel(mcx,mcy, TL, '180−α',  38, 17, -4));
    parts.push(inwardLabel(mcx,mcy, TR, '180−β',  24, 17,  0, -7));
    ...
    // Angle labels: α at rBR, 180−α at rTR
    const rcx=(rTL.x+rTR.x+rBR.x+rBL.x)/4, rcy=(rTL.y+rTR.y+rBR.y+rBL.y)/4;
    parts.push(inwardLabel(rcx,rcy, rBR, 'α',     22, 16,  2));
    parts.push(inwardLabel(rcx,rcy, rTR, '180−α', 16, 14,  3, -12));
    ...
    [Geo.segment(iTL,iTR),Geo.segment(iTR,iBR),Geo.segment(iBR,iBL),Geo.segment(iBL,iTL)].forEach(s=>
      parts.push(GeoRenderer.segment(s,{color:black,width:2})));
    // Blue diagonals
    parts.push(GeoRenderer.segment(Geo.segment(iTL,iBR),{color:blue,width:1.8}));
    parts.push(GeoRenderer.segment(Geo.segment(iTR,iBL),{color:blue,width:1.8}));
